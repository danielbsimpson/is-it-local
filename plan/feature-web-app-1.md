...
- **REQ-008**: The app MUST integrate an analytics provider and an error-monitoring provider, both configured via environment variables.
- **REQ-009**: The app MUST be deployable as a public preview with a documented build command and environment configuration.
...
| TASK-003 | Create `apps/web/src/lib/config.ts` reading `NEXT_PUBLIC_API_BASE_URL`, `NEXT_PUBLIC_MAP_TILE_URL`, `NEXT_PUBLIC_ANALYTICS_ID`, and `NEXT_PUBLIC_ERROR_DSN` from environment variables. | | |
...
| TASK-009 | Create `apps/web/.env.example` documenting `NEXT_PUBLIC_API_BASE_URL`, `NEXT_PUBLIC_MAP_TILE_URL`, `NEXT_PUBLIC_ANALYTICS_ID`, `NEXT_PUBLIC_ERROR_DSN`. | | |
...
| TASK-011 | Create `apps/web/src/lib/geocode.ts` exporting `geocode(query) -> {lat, lon}` using a configured geocoding endpoint, with graceful fallback when geolocation/geocoding fails. | | |
...
- GOAL-003: Add responsiveness verification, analytics, error monitoring, tests, and a public preview deployment.

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-018 | Verify and adjust responsive layout for 360px, 768px, and 1280px breakpoints across home, search, and detail pages. | | |
| TASK-019 | Create `apps/web/src/lib/analytics.ts` initializing the analytics provider from `NEXT_PUBLIC_ANALYTICS_ID` and exposing a `trackEvent(name, props)` function; wire page-view and search events. | | |
| TASK-020 | Create `apps/web/src/lib/monitoring.ts` initializing error monitoring from `NEXT_PUBLIC_ERROR_DSN` and hook it into the error boundary. | | |
...
| TASK-024 | Configure a public preview deployment (Vercel or equivalent) with build command `pnpm --filter web build` and required environment variables; document the resulting preview URL. | | |
...
- **ALT-005**: Building a bespoke analytics/monitoring solution — rejected in favor of configured third-party providers to reduce scope.
...
- **DEP-005**: A geocoding endpoint/provider reachable from the app for free-text location resolution.
- **DEP-006**: Analytics and error-monitoring provider accounts/keys.
- **DEP-007**: A hosting platform (e.g., Vercel) for the public preview deployment.
...
- **FILE-008**: `apps/web/src/lib/analytics.ts` — analytics initialization and event tracking.
- **FILE-009**: `apps/web/src/lib/monitoring.ts` — error-monitoring initialization.
...
- **ASSUMPTION-001**: The Phase 1 API is deployed and reachable at `NEXT_PUBLIC_API_BASE_URL`.
- **ASSUMPTION-002**: Search results include a distance value or the coordinates needed to compute one client-side.
- **ASSUMPTION-003**: A geocoding endpoint is available for free-text location input.
- **ASSUMPTION-004**: The chosen hosting platform supports pnpm monorepo builds with workspace filtering.
