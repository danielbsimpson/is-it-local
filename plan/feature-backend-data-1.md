...
This implementation plan operationalizes **Phase 1 — Backend & Data (API-first)** from [TODO.md](../TODO.md). It delivers a FastAPI backend, a PostgreSQL + PostGIS database with migrations, read/search/detail endpoints, data ingestion from Foursquare and OpenStreetMap/Overture, and an LLM-assisted enrichment pipeline that classifies business ownership with confidence scores and cited sources. Completion of this plan produces a running, tested API that serves classified business data, enabling Phase 2 (Web App) development. This plan depends on the completion of Phase 0 ([infrastructure-foundations-1.md](infrastructure-foundations-1.md)).
...
- **SEC-001**: All external API keys (`FOURSQUARE_API_KEY`, `LLM_API_KEY`, `SEARCH_API_KEY`, `DATABASE_URL`) MUST be read from environment variables and MUST NOT be hardcoded.
...
- **SEC-004**: The enrichment pipeline MUST enforce a per-provider rate limit and a configurable maximum spend guard to prevent runaway LLM/search costs.
...
| TASK-002 | Create `apps/api/app/config.py` defining a `Settings` class (pydantic-settings) reading `DATABASE_URL`, `FOURSQUARE_API_KEY`, `LLM_API_KEY`, `SEARCH_API_KEY`, `ENRICHMENT_MAX_SPEND_USD`, `PROVIDER_RATE_LIMIT_PER_MIN`. | | |
...
| TASK-011 | Create `infra/docker-compose.yml` defining services `db` (image `postgis/postgis:16-3.4`, healthcheck) and `api` (builds `apps/api`, depends_on `db`), with named volume `pgdata`. | | |
...
- GOAL-002: Implement idempotent data ingestion from Foursquare and OpenStreetMap/Overture with de-duplication and a repeatable seed job.
...
| TASK-015 | Create `apps/api/app/providers/foursquare.py` implementing `PlaceProvider` against the Foursquare Places API, mapping responses to `NormalizedPlace` (name, address, lat, lon, categories, provider_place_id). | | |
...
| TASK-020 | Create `docs/data-sources-compliance.md` documenting Foursquare and OpenStreetMap/Overture license and terms-of-use compliance for ingestion and redistribution. | | |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-022 | Create `packages/enrichment/pyproject.toml` declaring dependencies: `httpx`, `pydantic>=2`, `tenacity`, and a shared DB access layer or client to the API. | | |
| TASK-023 | Create `packages/enrichment/enrichment/search_client.py` implementing web-search retrieval of ownership signals via `SEARCH_API_KEY`, returning candidate `Source` objects (url, snippet, retrieved_at). | | |
| TASK-024 | Create `packages/enrichment/enrichment/llm_classifier.py` implementing a function `classify(business, sources) -> ClassificationResult` that calls the LLM (`LLM_API_KEY`) and returns `{classification, confidence, cited_source_ids}` constrained to the six enum values. | | |
| TASK-025 | Create `packages/enrichment/enrichment/guardrails.py` implementing a token-bucket rate limiter (`PROVIDER_RATE_LIMIT_PER_MIN`), a cumulative spend tracker enforcing `ENRICHMENT_MAX_SPEND_USD`, retries via `tenacity`, and an on-disk/DB response cache. | | |
...
- **DEP-004**: Valid `FOURSQUARE_API_KEY` for Foursquare Places API access.
- **DEP-005**: Valid `LLM_API_KEY` for the chosen LLM provider.
- **DEP-006**: Valid `SEARCH_API_KEY` for the chosen web-search provider.
...
- **FILE-012**: `apps/api/app/providers/base.py`, `foursquare.py`, `overture.py` — provider integrations.
...
- **TEST-009**: Guardrails halt processing when cumulative spend reaches `ENRICHMENT_MAX_SPEND_USD` and enforce the per-minute rate limit.
...
- **RISK-003**: Uncontrolled LLM/search usage could incur high costs; mitigated by the spend guard in `guardrails.py` (TASK-025).
...
- **ASSUMPTION-003**: Chosen LLM and web-search providers expose HTTP APIs usable via `httpx`.
- **ASSUMPTION-004**: Foursquare and OpenStreetMap/Overture licenses permit ingestion and redistribution of derived classification data (to be confirmed in TASK-020).
