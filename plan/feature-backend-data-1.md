...
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------- |
| TASK-001 | Create `apps/api/pyproject.toml` declaring dependencies: `fastapi`, `uvicorn[standard]`, `sqlalchemy>=2`, `geoalchemy2`, `alembic`, `psycopg[binary]`, `pydantic>=2`, `pydantic-settings`, `httpx`, `pytest`, `pytest-asyncio`.                                                               | ✅        | 2026-09-06 |
| TASK-002 | Create `apps/api/app/config.py` defining a `Settings` class (pydantic-settings) reading `DATABASE_URL`, `LLM_BASE_URL`, `LLM_MODEL`, `SEARXNG_BASE_URL`, `PROVIDER_RATE_LIMIT_PER_MIN`.                                                                                                       | ✅        | 2026-09-06 |
| TASK-003 | Create `apps/api/app/db.py` configuring the SQLAlchemy engine, `SessionLocal`, and a `get_db` FastAPI dependency.                                                                                                                                                                             | ✅        | 2026-09-06 |
| TASK-004 | Create ORM models in `apps/api/app/models/`: `business.py` (`Business`), `classification.py` (`OwnershipClassification`), `source.py` (`Source`), `community_submission.py` (`CommunitySubmission`), matching Phase 0 schemas; `Business.location` uses `geoalchemy2.Geography(POINT, 4326)`. | ✅        | 2026-09-06 |
| TASK-005 | Create Pydantic schemas in `apps/api/app/schemas/` mirroring the ORM models for request/response serialization, including a `ClassificationEnum` with the six required values.                                                                                                                | ✅        | 2026-09-06 |
| TASK-006 | Initialize Alembic in `apps/api/alembic/`; create migration `0001_enable_postgis` executing `CREATE EXTENSION IF NOT EXISTS postgis`.                                                                                                                                                         | ✅        | 2026-09-06 |
| TASK-007 | Create Alembic migration `0002_create_core_tables` creating tables `businesses`, `ownership_classifications`, `sources`, `community_submissions` with a GIST index on `businesses.location` and a unique constraint on `(provider, provider_place_id)` in `businesses`.                       | ✅        | 2026-09-06 |
| TASK-008 | Create `apps/api/app/repositories/business_repository.py` with functions `get_by_id`, `search(name, lat, lon, radius_m)` using `ST_DWithin`, and `upsert_by_provider_identity`.                                                                                                               | ✅        | 2026-09-06 |
| TASK-009 | Create `apps/api/app/routers/businesses.py` exposing `GET /businesses/{id}` (detail with classification + sources) and `GET /businesses/search` (query params `name`, `lat`, `lon`, `radius_m`).                                                                                              | ✅        | 2026-09-06 |
| TASK-010 | Create `apps/api/app/main.py` instantiating the FastAPI app, including the businesses router, enabling OpenAPI at `/docs` and `/openapi.json`, and a `GET /health` endpoint.                                                                                                                  | ✅        | 2026-09-06 |
| TASK-011 | Create `infra/docker-compose.yml` defining services `db` (image `postgis/postgis:16-3.4`, healthcheck), `api` (builds `apps/api`, depends_on `db`), and `searxng` (image `searxng/searxng`, JSON output format enabled) with named volume `pgdata`.                                           | ✅        | 2026-09-06 |
| TASK-012 | Create `apps/api/Dockerfile` (python:3.11-slim base) installing dependencies and running `uvicorn app.main:app`.                                                                                                                                                                              | ✅        | 2026-09-06 |
| TASK-013 | Create tests in `apps/api/tests/`: `test_health.py`, `test_business_detail.py`, `test_business_search.py` using `pytest` + `httpx` against a test database.                                                                                                                                   | ✅        | 2026-09-06 |
...
| Task     | Description                                                                                                                                                                                                               | Completed | Date |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---- |
| TASK-014 | Create `apps/api/app/providers/base.py` defining the `PlaceProvider` abstract interface with method `fetch_places(bbox: BoundingBox) -> list[NormalizedPlace]`.                                                           | ✅        | 2026-09-20 |
| TASK-015 | Create `apps/api/app/providers/openstreetmap.py` implementing `PlaceProvider` against the OpenStreetMap Overpass API, mapping elements to `NormalizedPlace` (name, address, lat, lon, categories, provider_place_id).     | ✅        | 2026-09-20 |
| TASK-016 | Create `apps/api/app/providers/overture.py` implementing `PlaceProvider` reading OpenStreetMap/Overture place data and mapping to `NormalizedPlace`.                                                                      | ✅        | 2026-09-20 |
| TASK-017 | Create `apps/api/app/services/ingestion_service.py` orchestrating provider fetches and calling `business_repository.upsert_by_provider_identity` for idempotent writes.                                                   | ✅        | 2026-09-20 |
| TASK-018 | Implement de-duplication in `apps/api/app/services/dedup_service.py` matching records across providers by normalized name + geospatial proximity (`ST_DWithin` within 50 meters) and merging into a canonical `Business`. | ✅        | 2026-09-20 |
| TASK-019 | Create CLI seed script `apps/api/app/cli/seed.py` (invocable via `python -m app.cli.seed --provider <name> --bbox <coords>`) that runs ingestion and de-duplication.                                                      | ✅        | 2026-09-20 |
| TASK-020 | Create `docs/data-sources-compliance.md` documenting OpenStreetMap (ODbL) and Overture license and terms-of-use compliance for ingestion and redistribution.                                                              | ✅        | 2026-09-20 |
| TASK-021 | Create tests `apps/api/tests/test_ingestion_idempotent.py` (asserts re-running seed does not duplicate records) and `apps/api/tests/test_dedup.py`.                                                                       | ✅        | 2026-09-20 |
